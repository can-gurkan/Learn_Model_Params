...
to setup
  ...
  update-display
  set stopped? false
  set people-extinct? false
  set virus-extinct? false
  reset-ticks
end
...
to go
  ...
  update-global-variables
  update-display
  if (ticks > 2000) or (%infected = 0.0) or (count turtles = 0)
  [
    if count turtles = 0
    [
      set people-extinct? true
    ]
    if %infected = 0.0
    [
      set virus-extinct? true
    ]
    set stopped? true
    stop
  ]
...
to infect ;; turtle procedure
  ask other turtles in-radius transmission-radius with [ not sick? and not immune? ]
...
to recover-or-die ;; turtle procedure
  if sick-time > duration                        ;; If the turtle has survived past the virus' duration, then
    [ ifelse random-float 100 < chance-recover   ;; either recover or die
      [ become-immune ]
      [
        set virus-deaths virus-deaths + 1
        die
      ]
  ]
end
...
<experiments>
  <experiment name="Test" repetitions="100" runMetricsEveryStep="false">
    ...
    <exitCondition>stopped?</exitCondition>
    <metric>virus-deaths</metric>
    <metric>ticks</metric>
    <metric>people-extinct?</metric>
    <metric>virus-extinct?</metric>
    ...
    <enumeratedValueSet variable="transmission-radius">
      <value value="2"/>
    </enumeratedValueSet>
